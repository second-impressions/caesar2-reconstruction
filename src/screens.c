...
#define ICON_STRIP_LAST_COST old_cost
#else
#define ICON_STRIP_LAST_TEXT request_message.cached_text_id
#define ICON_STRIP_LAST_COST request_message.cached_cost
#endif
...
void show_icon_strip(void)
{
    ...
    if (cost_exists != 0) {
        ...
        ICON_STRIP_LAST_COST = cost;
    } else {
        ICON_STRIP_LAST_COST = -1;
        if (SCREEN_MAP_MODE == 0) {
            if (selected_flag != 0)
...
void show_query_house_advice(void)
{
    int next_promotion_level;
    int demote_level;
    int advice_id;

#if PLATFORM_WINDOWS
#define HOUSE_PROMOTION_LEVELS (((signed char *)province_completion_to_promotion) + 4)
#else
#define HOUSE_PROMOTION_LEVELS (((signed char *)promotion_av_levels) + 0x186)
#endif
    next_promotion_level = HOUSE_PROMOTION_LEVELS[q_type * 2];
