                         ...
                         unsigned char *sprite_data_ptr, int sprite_kind)
{
    ...
    unit_list[created_unit_no].stage_slot = stance;
    if (bat_control != 0) unit_list[created_unit_no].morale_a += bat_odds * 5;
    else unit_list[created_unit_no].morale_a -= bat_odds * 5;
    ...
    unit_list[created_unit_no].start_men = target_men;
    unit_list[created_unit_no].current_men = unit_list[created_unit_no].start_men;
    ...
    if (bat_control == 0 && unit_rank == 1) {
        if (bat_enemy_left_flank_unit == 0 && x <= 0x1a) {
            unit_list[created_unit_no].flank_pending = 1; bat_enemy_left_flank_unit = 1;
        }
...
#if PLATFORM_WINDOWS
#define SF13_MISSILE_MAX 0x30
#define SF13_TARGET_RANGE 0xf
#else
#define SF13_MISSILE_MAX 0x20
#define SF13_TARGET_RANGE 5
#endif
...
void sf13_autofire_missile(void)
{
    ...
    get_fig_still_image();
    figure_list[figure_no].missile_max = SF13_MISSILE_MAX;
    figure_list[figure_no].missile_timer++;
    if (figure_list[figure_no].missile_timer
        > figure_list[figure_no].missile_max) {
        figure_list[figure_no].missile_timer = 0;
        if (find_nearest_target(SF13_TARGET_RANGE) != 0) {
            figure_list[figure_no].missile_target = enemy_figure;
...
#undef SF13_MISSILE_MAX
#undef SF13_TARGET_RANGE
...
void set_ai_unit_withdraw(int offset_x, int offset_y)
{
    ...
    for (temp_figure = unit_list[temp_unit].first_figure;
        ...
        if (figure_list[temp_figure].exists != 0) {
            ...
            figure_list[temp_figure].prev_grid_x = figure_list[temp_figure].grid_x + offset_x;
            figure_list[temp_figure].prev_grid_y = figure_list[temp_figure].grid_y + offset_y;
        }
    }
}
...
void raise_all_units_morale(int skip_type, int morale_a_delta, int morale_b_delta)
{
    ...
    for (unit_idx = 1; unit_idx < 0x33; unit_idx++) {
        if (unit_list[unit_idx].exists != 0 &&
            unit_list[unit_idx].type != skip_type) {
            unit_list[unit_idx].morale_a += morale_a_delta;
            unit_list[unit_idx].morale_b += morale_b_delta;
